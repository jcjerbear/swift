...
// appropriately.
// After calling this function, not only do we have a ValueKind as the return value, but
// also the Cast node(s) for `instr` are created. 
...
ValueKind WALAWalker::getInstrValueKindInfo(SILInstruction &instr, WALAIntegration &wala, 
  ...
  // This is an important debugging info. It allows us to track the relationships between one instruction and another
...
// Here is how the CAst nodes are created:
// 1. A SILInstruction corresponds to one or more CAst nodes. If it corresponds to more than one CAst nodes, some CAst nodes may be others' parents. 
// 2. After each call to getInstrValueKindInfo(), all the "root level" CAst nodes will be inserted into `nodeList`
// 3. `nodeMap` is a temporal storage (hashmap) for CAst nodes. It allows getInstrValueKindInfo() to link the parent CAst node with its children. 
// 4. The lifetime of both `nodeList` and `nodeMap` is a SILBasicBlock
// 5. During the iteration over SILInstructions, previous CAst nodes may be removed from `nodeList` if they are children of new CAst nodes
// 6. At the very end of each SILBasicBlock, all the remaining CAst nodes in `nodeList` will be gathered into a WALA BLOCK_STMT. The first CAst node in the BLOCK_STMT is lablled by a LABEL_STMT using `labeller`
// 7. Each SILFunction generates a `blockStmtList`, which contains all the BLOCK_STMT created. Currently, the `blockStmtList` is simply printed and deleted afterwards.
// 8. A global `symbolTable` is used for the entire source program
void WALAWalker::analyzeSILModule(SILModule &SM) {
  ...
  for (auto func = SM.begin(); func != SM.end(); ++func) {
    ...
    // Print out the memory address for debugging purpose. This allows us to track the relationship between one SILFunction and another
    llvm::outs() << "SILFunction: " ;
    llvm::outs() << &*func << "\n";

    // Print this SILFunction in SIL format for debugging purpose.
    func->print(llvm::outs(), true);

    // Iterate over SILBasicBlocks
    for (auto bb = func->begin(); bb != func->end(); ++bb) {
      // Print out the memory address for debugging purpose. This allows us to track the relationship between one SILBasicBlock and another
      ...
      for (auto instr = bb->begin(); instr != bb->end(); ++instr) {
        ...
        instrInfo.instrKind = getInstrValueKindInfo(*instr, wala, nodeMap, nodeList, symbolTable, labeller); // this is the call that creates CAst nodes
